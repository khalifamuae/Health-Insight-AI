...
function buildFriendlyExplanation(test: AllTestData, isArabic: boolean): string {
  ...
  const simpleDesc = isArabic ? test.descriptionAr : test.descriptionEn;
  const simpleDescLine = simpleDesc
    ? (isArabic ? `وظيفة الفحص: ${simpleDesc}` : `What this test does: ${simpleDesc}`)
    : null;

  const knowledgeSection = getKnowledgeSection(test.testId, test.category, isArabic);
  const sections = [
    simpleDescLine,
    knowledgeSection || roleText,
    valueLine,
    rangeLine,
    statusLine,
    adviceLine,
